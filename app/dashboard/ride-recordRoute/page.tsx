...
export default function RideRecordRouteManagement() {
  const [routes, setRoutes] = useState<RideRecordRoute[]>([]);
  const [users, setUsers] = useState<User[]>([]);
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [form] = Form.useForm();
  const [editingRoute, setEditingRoute] = useState<RideRecordRoute | null>(null);
  ...
  const fetchRoutes = async (
    ...
    try {
      ...
      const response = await fetchWithAuth(
        `/api/ride-recordRoutes/page?page=${page}&pageSize=${pageSize}&keyword=${keyword}&type=${type}&sort=${sort}`
      );
      setWait(false);
  ...
  const handleEdit = (route: RideRecordRoute) => {
  ...
  const handleDelete = async (id: string) => {
    Modal.confirm({
      title: "确认删除",
      content: "您确定要删除该骑行记录吗？",
      okText: "确认",
      cancelText: "取消",
      onOk: async () => {
        setWait(true);
        try {
          const response = await fetchWithAuth(`/api/ride-recordRoutes/${id}`, {
            method: "DELETE",
          });
          ...
          message.success("骑行记录删除成功");
        } catch (error) {
          console.error("Error deleting route:", error);
          message.error("删除骑行记录失败");
        }
      },
    });
  };
  ...
  const handleOk = async () => {
    try {
      ...
      const response = editingRoute
        ? await fetchWithAuth(`/api/ride-recordRoutes/${editingRoute.id}`, {
            method: "PUT",
            headers: {
        ...
        : await fetchWithAuth("/api/ride-recordRoutes", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
      ...
      message.success(editingRoute ? "骑行记录更新成功" : "骑行记录添加成功");
    } catch (error) {
      console.error("Error submitting form:", error);
      message.error("保存骑行记录失败");
    }
  };
  ...
  const columns = [
    ...
    },
    {
      title: "操作",
      key: "action",
      render: (_: any, record: RideRecordRoute) => (
        <Space size="middle">
  ...
  const handleBatchDelete = async () => {
    try {
      setWait(true);
      const response = await fetchWithAuth(
        `/api/ride-recordRoutes/batch-delete`,
        {
          method: "DELETE",
  ...
  return (
    <div>
      ...
      />
      <Modal
        title={editingRoute ? "编辑骑行记录" : "添加骑行记录"}
        open={isModalVisible}
        onOk={handleOk}
        onCancel={() => {
          setIsModalVisible(false);
