...
export default function UserPublishRouteManagement() {
  ...
  const fetchRoutes = async (
    ...
    try {
      ...
      const response = await fetchWithAuth(
        `/api/user-publish-routes/page?page=${page}&pageSize=${pageSize}&keyword=${keyword}&type=${type}&sort=${sort}`
      );
      setWait(false);
  ...
  const handleDelete = async (id: string) => {
    Modal.confirm({
      ...
      onOk: async () => {
        setWait(true);
        try {
          const response = await fetchWithAuth(`/api/user-publish-routes/${id}`, {
            method: "DELETE",
          });
  ...
  const handleOk = async () => {
    try {
      ...
      const response = editingRoute
        ? await fetchWithAuth(`/api/user-publish-routes/${editingRoute.id}`, {
            method: "PUT",
            headers: {
        ...
        : await fetchWithAuth("/api/user-publish-routes", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  const handleBatchDelete = async () => {
    try {
      setWait(true);
      const response = await fetchWithAuth(`/api/user-publish-routes/batch-delete`, {
        method: "DELETE",
